...
public class DBPage extends PayMatePage {

  // logging facilities
  protected static final ErrorLogStream dbg=new ErrorLogStream(DBPage.class.getName());

  private static final JdbcGatewayOp dfltOp = new JdbcGatewayOp(JdbcGatewayOp.overview);

  public DBPage(Element e, String loginInfo) {
  ...
  public static final String key() {
    return PayMatePage.key(DBPage.class);
  }

  public static final Element generatePage(HttpServletRequest req, UserSession usession, String pathAdd, ColorScheme cs, Element [] otherElements) {
    ElementContainer ec = new ElementContainer();
    ResultSet rs = null;
    String base = req.getServletPath();
    dbg.VERBOSE("Query: " + req.getQueryString());
    base += (Safe.NonTrivial(pathAdd)) ? pathAdd : "?";
    DatabaseMetaData dbmd = usession.db.getDatabaseMetadata();
    String opStr = Safe.TrivialDefault(req.getParameter("op"), dfltOp.Image());
    JdbcGatewayOp op = new JdbcGatewayOp();
    op.setto(opStr);
    // check to see if we can just continue a previous query ...
    String key = req.getParameter("k");
    if(key != null) {
      op.setto(op.disptable);
    }
    String sessionid = usession.sessionid();
    ec.addElement(printHeader(base, dbmd));
    switch(op.Value()) {
      case JdbcGatewayOp.profile: {
        // profile does not need pagination (finite length)
        String table = (String) req.getParameter("table");
        ec.addElement(profileTable(usession.db, cs, sessionid, table));
        if(Safe.NonTrivial(table)) {
          ec.addElement(printCapabilities(dbmd, cs, sessionid, table));
        }
        ec.addElement(printTopLink(base));
      } break;
      case JdbcGatewayOp.capabilities: {
        // capabilities do not need pagination (finite length) ??? ---
        ec.addElement(printCapabilities(dbmd, cs, sessionid, null))
          .addElement(printTopLink(base));
      } break;
      case JdbcGatewayOp.disptable: {
        // disptable DOES need pagination (unknown length)
        // Write database table
        boolean isQueryRes = false;
        String table = (String) req.getParameter("table");
        String query = (String) req.getParameter("query");
        TableGen tablegen = (key != null) ? TableGen.getFromKey(key) : null;
        if(tablegen == null) {
          QueryString queryStr = null;
          if(Safe.NonTrivial(query)) {
            query = query.trim();
            queryStr = QueryString.Clause(query);
            isQueryRes = true;
          } else {
            queryStr = QueryString.SelectAllFrom(table);
            query = queryStr.toString().trim();
          }
          dbg.VERBOSE("SQL: " + queryStr);
          String sqlop = query.substring(0, 6);
          dbg.VERBOSE("SQL op: " + sqlop);
          Exception mye = null;
          long duration = -1;
          if ("SELECT".equalsIgnoreCase(sqlop)) {
            // this is eligible for pagination
            try {
              long dur1 = Safe.utcNow();
              rs = usession.db.getResultSet(usession.db.query(queryStr, true));
              duration = Safe.utcNow() - dur1;
            } catch (Exception trash1) {
              dbg.Caught("Exception occurred attempting to run the query: " + queryStr, trash1);
              mye = trash1;
            }
            if((rs == null) || (mye!=null)) {
              ec.addElement("Possible error performing the query: " + queryStr + "<BR><PRE>"+dbg.resolveExceptions(mye)+"</PRE><BR>");
            } else {
              String title = isQueryRes ? ("Result of query: " + queryStr) : ("Table: " + table);
              tablegen = new AnyDBTableGen(title, cs, rs, null, base, 15, sessionid);
            }
          } else {
            int i = -1;
            ...
            try {
              long dur1 = Safe.utcNow();
              i = usession.db.update(queryStr, id, true);
              duration = Safe.utcNow() - dur1;
            } catch (Exception e) {
              dbg.Caught(e);
              mye = e;
            }
            String more = ((i>-1) && (mye==null)) ?
              "Insert/update OK [" + i + "]<BR>" + (id.isValid() ? "Inserted id: "+id.toString()+"<BR>": ""):
              "Possible error performing the insert/update [" + i + "]: "  + queryStr + "<BR><PRE>" + dbg.resolveExceptions(mye)+"</PRE><BR>";
            ec.addElement(more + "Took " + duration + " milliseconds.");
          }
        }
        if(tablegen != null) {
          ec.addElement(tablegen);
        }
        String dfltQuery = isQueryRes ? query : QueryString.SelectAllFrom(Safe.TrivialDefault(table, "table")).toString();
        ec.addElement(printSQLForm(base, dfltQuery, otherElements));
        ec.addElement(printTopLink(base));
      } break;
      default:
      case JdbcGatewayOp.overview: {
        ...
        // Write list of tables (and other stuff)
          ec.addElement(ProfileDBTableGen.output(usession.db.getTableList(), cs, base, sessionid))
            .addElement(BRLF)
            .addElement(printSQLForm(base, QueryString.SelectAllFrom("table").toString(), otherElements))
            .addElement(BRLF);
      } break;
    }
    usession.db.closeStmt(usession.db.getStatement(rs));
    return ec;
  }
  ...
  // Generates HTML link to top page.
  protected static final Element printTopLink(String base) {
    ElementContainer ec = new ElementContainer();
    ec.addElement(BRLF)
  ...
  // Generates HTML form for entering SQL expression.
  protected static final Element printSQLForm(String base, String dfltQuery, Element [] otherElements) {
    Element [] formItems = {
      new Input().setType(Input.SUBMIT).setValue("Submit SQL Query"),
      new BR(),
      new Input(Input.HIDDEN, "op", "disptable"),
      new TextArea("query", 16, 80).addElement(dfltQuery),
    };
    Form f = new Form(base, Form.POST).addElement(LF);
    addElems2Form(f, formItems);
    addElems2Form(f, otherElements);
    ElementContainer ec = new ElementContainer();
    ec.addElement(BRLF)
      .addElement(BRLF)
      .addElement(f);
  ...
  public static final Element profileTable(PayMateDB db, ColorScheme cs, String sessionid, String tablename) {
    ElementContainer ec = new ElementContainer();
    ec.addElement(ProfileTableGen.output(db, cs, tablename, sessionid));
    return ec;
  }

  public static final Element printCapabilities(DatabaseMetaData dbmd, ColorScheme cs, String sessionid, String tablename) {
    ElementContainer ec = new ElementContainer();
    if((tablename != null) && !Safe.NonTrivial(tablename)) {
      tablename = null;
    }
    boolean tabled = Safe.NonTrivial(tablename);
    if(!tabled) {
      ec.addElement(settings(dbmd, cs, sessionid)).addElement(BRLF);
    }
    ...
    for(int i = 0; i < cap.numValues(); i++) {
      ...
      try {
        ...
        if(rs != null) {
          ec.addElement(AnyDBTableGen.output(cap.Image(), cs, rs, null, sessionid)).
             addElement(BRLF);
        }
        //DBMacros.closeRS(rs);
  ...
  private static final Element settings(DatabaseMetaData dbmd, ColorScheme cs, String sessionid) {
    ElementContainer ec = new ElementContainer();
    try {
    String tdata[][] = {
...
*/
      ec.addElement(ArrayTableGen.output("Capabilities", cs, tdata, headers, sessionid));
    } catch (SQLException e) {
...
class Capabilities extends TrueEnum {
  ...
  static final TextList myText = new TextList(texts);
