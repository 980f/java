...
public
  ...
  // the generic public defaults
  public final static int DEFAULTFILEMODE = 0100644;
  public final static int DEFAULTDIRMODE = 040755;
  // these are set FOR THIS OBJECT by the
  //   TarEntry(File file, int filemode, int dirmode) constructor
  public int defaultFileMode = DEFAULTFILEMODE;
  public int defaultDirMode = DEFAULTDIRMODE;
  ...
  public TarEntry(File file) throws InvalidHeaderException {
    this.initialize();
    this.getFileTarHeader(this.header, file); // uses the defaults in this guy
  }
   ...
   * @param file The file that the entry represents.
   */
  public TarEntry(File file, int filemode, int dirmode) throws InvalidHeaderException {
    this.initialize();
...
//    System.out.println("defaultDirMode is " +
//                       net.paymate.util.BaseConverter.itoa(defaultDirMode, 8) +
//                       "; defaultFileMode is " + net.paymate.util.BaseConverter.itoa(defaultFileMode, 8));
    if (file.isDirectory()) {
      hdr.mode = defaultDirMode;
      hdr.linkFlag = TarHeader.LF_DIR;
      if (hdr.name.charAt(hdr.name.length() - 1) != '/'){
        hdr.name.append("/");
      }
    } else {
      hdr.mode = defaultFileMode;
      hdr.linkFlag = TarHeader.LF_NORMAL;
    }
//    System.out.println("mode set to " + net.paymate.util.BaseConverter.itoa(hdr.mode, 8));
  ...
  public TarEntry[] getDirectoryEntries() throws InvalidHeaderException {
    ...
    for (int i = 0; i < list.length; ++i) {
      result[i] = new TarEntry (new File(this.file, list[i]), defaultFileMode, defaultDirMode);
    }
   ...
   * alh: only 24 bits is used, purged long's as they are not atomic
   * @param buf The tar entry's header buffer.
   * @return The computed checksum.
   */
  public int computeCheckSum(byte[] buf) {
    int sum = 0;
  ...
  public void writeEntryHeader(byte[] outbuf) {
    ...
    int checkSum = this.computeCheckSum(outbuf);
  ...
  public void parseTarHeader(TarHeader hdr, byte[] header) throws InvalidHeaderException {
    ...
    if(hdr.size<=0){
    System.err.println("zero size while parsing header");
    }
  ...
  public void nameTarHeader(TarHeader hdr, String name) {
    ...
    hdr.name = new StringBuffer(name);
    hdr.mode = isDir ? defaultDirMode: defaultFileMode;
    hdr.userId = 0;
    hdr.groupId = 0;
    hdr.size = 0;
    hdr.checkSum = 0;
...
//paymate revision tracking
//$Log: TarEntry.java,v $
//Revision 1.2  2004/03/02 00:40:40  andyh
//javaruf client with screaming debug
//
//$Id: TarEntry.java,v 1.2 2004/03/02 00:40:40 andyh Exp $
//$Source: /cvs/src/net/paymate/util/compress/tar/TarEntry.java,v $
