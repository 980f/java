...
public class TarInputStream extends FilterInputStream {
   ...
   *
   * @param factory an object that can create TarEntry's given bytes.
   */
  ...
  public TarEntry getNextEntry() throws IOException {
    ...
    if(this.currEntry != null) {
      if(entrySize<=0){
      System.err.println("zero size entry in  getNextEntry skipper");
      }
      ...
      System.err.println("nothing to skip");
      }

      this.readBuf = null;//why is this nulled??
    }
    ...
    if(this.hasHitEOF) {
      ...
      try {
        ...
        if(! (headerBuf[257] == 'u' && headerBuf[258] == 's' && headerBuf[259] == 't' && headerBuf[260] == 'a' && headerBuf[261] == 'r')) {
          throw new InvalidHeaderException("header magic is not 'ustar', but '" + headerBuf[257] + headerBuf[258] + headerBuf[259] + headerBuf[260] + headerBuf[261] + "', or (dec) " + ( (int) headerBuf[257]) + ", " + ( (int) headerBuf[258]) + ", " + ( (int) headerBuf[259]) +
        ...
        if(entrySize<=0){
        System.err.println("zero entry size in getNextEntry");
        }
...
// original   byte[] buf = new byte[32 * 1024];
//alh: optimized for small files
    byte[] buf = new byte[Math.min(this.entrySize, 32768)]; //arbitrary 32k pacing to speed up big files
...
//Paymate.net:
//$Log: TarInputStream.java,v $
//Revision 1.2  2004/03/02 00:40:39  andyh
//javaruf client with screaming debug
//
//$Id: TarInputStream.java,v 1.2 2004/03/02 00:40:39 andyh Exp $