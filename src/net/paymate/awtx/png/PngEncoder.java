 ...
 * which can be saved as a PNG file or sent to a client by a servlet.
 * The Image is presumed to use the DirectColorModel; that is,
 * pixels are four-byte ints containing alpha channel, red, green, and blue information.
 * Thus, this PNG encoder creates PNGs with a color type of 2 (that is, RGB triples) and
 * a bit depth of 8, optionally encoding alpha channel information. This class does not
 * work with an IndexColorModel image. <br><br>
 *
 * Image data may be pre-filtered using the "sub" (previous byte) and "up" (previous
 * row) filters. <br><BR>
 *
 * You may also set the compression level (0 through 9) as used in
 * java.utils.zip.Deflater <br><br>
 *
...
//    }
    // +_+ coersion.  Do we have a class that does this yet?
  ...
  protected int writeBytes( byte[] data, int offset ) {
    return writeBytes( data, data.length, offset );
  }
